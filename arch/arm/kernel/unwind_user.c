...
static struct unwind_idx *search_index_safe(unsigned long addr,
				       struct unwind_idx *first,
				       struct unwind_idx *last)
{
	pr_debug("%s(%08lx, %p, %p, fa:%lx, la:%lx)\n", __func__, addr, first, last, prel31_to_addr(&first->addr), prel31_to_addr(&last->addr));

	if (addr < prel31_to_addr(&first->addr)) {
		pr_debug("unwind: Unknown symbol address %08lx\n", addr);
		return NULL;
	} else if (addr >= prel31_to_addr(&last->addr))
		return last;

	while (first < last - 1) {
		struct unwind_idx *mid = first + ((last - first + 1) >> 1);

		if (addr < prel31_to_addr(&mid->addr))
			last = mid;
		else
			first = mid;
	}
...
static struct unwind_idx *unwind_find_idx_user(unsigned long addr, struct bt_elf_cache *ep)
{
