...
#Include perf in linux-tools-common package.
do_tools_perf=true
...
install-tools: toolslib = $(CURDIR)/debian/$(toolspkg)/usr/lib
...
# Build and include perf binaries to linux-tools-common package
ifeq ($(do_tools_perf),true)
	install -d $(toolslib)
	cd $(builddir)/tools/tools/perf && make
	install -m755 $(builddir)/tools/tools/perf/perf $(toolsbin)/perf
	install -m755 $(builddir)/tools/tools/perf/perf-archive $(toolsbin)/perf-archive
	install -m755 $(builddir)/tools/tools/perf/python/perf.so $(toolslib)/perf.so
