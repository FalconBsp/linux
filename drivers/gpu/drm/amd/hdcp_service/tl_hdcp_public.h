//  Copyright 2015(c), Advanced Micro Devices, Inc. (unpublished)
//
...
//
//  Module           : tl_hdcp_public.h
//
//  Classification   : Confidential
//
//  Description      : This contains the public declartions used for interfacing
//                     with the HDCP trustlet
//
...
#ifndef __tl_hdcp_public__dot__h__
#define __tl_hdcp_public__dot__h__
...
#define HDCP2_DATA_INPUT_SIZE_16_IN_uint8_tS    16
...
typedef struct _HDCP2_RECEIVER_MSG
{
	...
	union
	{
		...
		struct _RepeaterAuth_Send_ReceiverID_List_Ver2_1           // This message is HDCP2.1 standard specfic
		{
			...
			uint8_t aucVprime[HDCP2_DATA_INPUT_SIZE_16_IN_uint8_tS];  // Variable V'  128 bit LSB value
			uint8_t aucReceiver_ID[HDCP2_MAX_DEVICE_COUNT][5];     // Receiver ID list
...
typedef struct _HDCP2_TRANSMITTER_MSG
{
	...
	union
	{
		...
		{
			uint8_t aucLprime[HDCP2_DATA_INPUT_SIZE_16_IN_uint8_tS];                           // LSB 128 bit value for L'
		} RTTChallange;

		struct _RepeaterAuth_Send_Ack
		{
			uint8_t aucVprime[HDCP2_DATA_INPUT_SIZE_16_IN_uint8_tS];                            // MSB 128 bit V value
		} RepeaterAuthSendAck;
...
typedef enum _HDCP_14_REQUEST_CODE
{
	HDCP_14_COMMAND_CREATE_SESSION     = 0x01,
	HDCP_14_COMMAND_DESTROY_SESSION     = 0x02,
	HDCP_14_COMMAND_SEND_DATA          = 0x03
} HDCP_14_REQUEST_CODE;

typedef struct _HDCP_14_IN_MSG
{
	uint8_t ulDisplayIndex;
	uint8_t bCommandCode; /* HDCP_14_COMMAND_CODE */
	uint8_t buffer[8];
...
typedef enum _HDCP_14_REQUEST_TYPE
{
	REQUESTING_NOTHING       = 0x01,
	REQUESTING_BKSV          = 0x02,
	REQUESTING_R_NOT         = 0x03,
	REQUESTING_KSVLIST_READY = 0x04,
	REQUESTING_KSVLIST       = 0x05
} HDCP_14_REQUEST_TYPE;

typedef struct _HDCP_14_OUT_MSG
{
	uint8_t bResponseCode; /* HDCP_14_RESPONSE_CODE */
	uint8_t bRequestType; /* HDCP_14_REQUEST_TYPE */
	uint32_t ulMisc;
	union
	{
		struct {
		...
		uint8_t bGenericOutputBuffer[16];
	};
...
typedef struct _HDCP_TCI
{
	...
	uint16_t               sCommand; /* enum TL_HDCP_COMMAND_CODE */
	uint8_t                p1;
	uint8_t                p2;
	uint8_t                p3;
	union
