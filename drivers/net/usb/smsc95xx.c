...
#ifdef CONFIG_SNSC_USB_NET_SMSC95XX_GET_OF_MAC_ADDRESS
#include <linux/of.h>
#include <linux/of_net.h>
static int smsc95xx_get_of_mac_address(unsigned char *dev_addr)
{
	...
	prop = of_get_property(np, "ethernet0", NULL);
	if (!prop) {
		of_node_put(np);
		return -1;
	}
	np2 = of_find_node_by_path(prop);
	of_node_put(np);
	if (!of_device_is_compatible(np2, "smsc,lan95xx"))
		goto Err;

	mac = of_get_mac_address(np2);
	if (!mac)
		goto Err;

	memcpy(dev_addr, mac, ETH_ALEN);
	of_node_put(np2);
	return 0;
Err:
	of_node_put(np2);
...
#ifdef CONFIG_SNSC_USB_NET_SMSC95XX_GET_OF_MAC_ADDRESS
	if (!smsc95xx_get_of_mac_address(dev->net->dev_addr))
		return;
#endif
...
#ifdef CONFIG_ARCH_OMAP4
static int omap_netdev_registerd;

int
usbnet_smsc95xx_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	if(register_netdevice_notifier(&omap_panda_netdev_notifier))
		printk("%s : failed to get ethernet mac address from die id, random id will be generated\n",__func__);
	else
		omap_netdev_registerd = 1;
	return usbnet_probe(udev, prod);
}

void usbnet_smsc95xx_disconnect (struct usb_interface *intf)
{
	if(omap_netdev_registerd)
		unregister_netdevice_notifier(&omap_panda_netdev_notifier);
...
#ifdef CONFIG_ARCH_OMAP4
	.disconnect	= usbnet_smsc95xx_disconnect,
#else
