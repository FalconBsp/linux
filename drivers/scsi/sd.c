...
#if defined(CONFIG_SNSC_SCSI_PARAM_SD_TIMEOUT)
static unsigned int snsc_sd_timeout = 30;
module_param(snsc_sd_timeout, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(snsc_sd_timeout, "SCSI disk timeout");
#define SD_TIMEOUT		(snsc_sd_timeout * HZ)
#else
...
static int sd_open(struct block_device *bdev, fmode_t mode)
{
...
#if !defined(CONFIG_SNSC_SCSI_SD_NEED_REVALIDATE)
...
#else
	{
		...
		result = check_disk_change(bdev);
		if (!result && sdkp->need_revalidate) {
			/* ref. block_dev.c: check_disk_change() */
...
#if defined(CONFIG_SNSC_SCSI_SD_NEED_REVALIDATE)
	/*
	 * If the media is changed, we need to revalidate at sd_open.
	 */
	if (sdp->changed)
		sdkp->need_revalidate = 1;
#endif
out:
	/*
...
#if !defined(CONFIG_SNSC_SCSI_MESSAGE_SILENT)
	if (res) {
...
#if !defined(CONFIG_SNSC_SCSI_MESSAGE_SILENT)  /* TARGET */
			sd_printk(KERN_NOTICE, sdkp,
...
#if defined(CONFIG_SNSC_SCSI_SD_CHK_SYNCHRONIZE_CACHE_SUPPORT)
		/* try to send SYNCHRONIZE CACHE command */
		if (sdkp->WCE && (sd_sync_cache(sdkp) != 0)) {
			sd_printk(KERN_ERR, sdkp,
				"WCE is valid, but SYNCHRONIZE CACHE command"
				" failed\n");
			sdkp->WCE = 0;
		}
#endif
...
#if defined(CONFIG_SNSC_SCSI_SD_RW10_DEFAULT)
		sdp->use_10_for_rw = 1;
#endif
	}
...
#if defined(CONFIG_SNSC_SCSI_SD_CHK_SYNCHRONIZE_CACHE_SUPPORT)
	/* try to send SYNCHRONIZE CACHE command */
	if (!sdp->skip_ms_page_8 && !sdkp->WCE && (sd_sync_cache(sdkp) == 0)) {
		printk(KERN_DEBUG "%s WCE is invalid, but SYNCHRONIZE CACHE "
		       "command success\n", disk->disk_name);
		sdkp->WCE = 1;
	}
#endif
...
static void sd_probe_async(void *data, async_cookie_t cookie)
{
	...
	gd->minors = SD_MINORS;
	sdp->sdev_dev.devt = MKDEV(gd->major, gd->first_minor);
...
#if defined(CONFIG_SNSC_SCSI_SD_NEED_REVALIDATE)
#if defined(CONFIG_SNSC_SCSI_SD_REUSE_INDEX)
	sdkp->need_revalidate = 1;
#else
	sdkp->need_revalidate = 0;
#endif
#endif
#if defined(CONFIG_SNSC_SCSI_PROC_DEVICE_NAME)
	strncpy(sdp->device_name, gd->disk_name, BDEVNAME_SIZE);/* FES patch */
#endif
...
#if defined(CONFIG_SNSC_SCSI_SD_CHK_SYNCHRONIZE_CACHE_SUPPORT)
	if (sdkp->WCE)
		sdev_printk(KERN_NOTICE, sdp, "WCE %s\n", gd->disk_name);
#endif
